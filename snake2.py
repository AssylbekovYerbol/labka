...
class Point:
    ...
    def generate(): #from all cells deleting "bad" cells and choosing random for the "good" ones
        global free
...
while not done:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if snake.dy != 1 and event.key == pygame.K_UP: #can't move on opposite direction
                snake.dx = 0
                snake.dy = -1
    ...
    if snake.check(food):
        ...
        buf = snake.body[0]
    snake.move()
    if f == True and len(g) == 1: #handle the case when len(body) = 1
        snake.body.append(buf)
    
    #count number of same points
    m = {}
    ...
    if m[(g[0].X, g[0].Y)] > 2: #checking for collison, count does not work :(
        print("SCORE:", score)
        print("LEVEL:", LVL // 3)
    ...
    TMR += 1
    if TMR == cur * FPS: #regenerate food after cur * FPS ticks
        food.reg()
        TMR = 0
        cur = -1
